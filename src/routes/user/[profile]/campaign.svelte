<script lang="ts">
	...
	let isLoading = false;
	async function updateData(campaign: string) {
		if (isLoading) {
			return;
		}
	...
	const categories = [
		...
		{ name: 'Campaign', url: '/campaign', current: true }
	];
	...
	const onMountFunction = async () => {
		...
		let campaignRes = await AppwriteService.getCampaignMaps();

		for (const map of campaignRes) {
			if (!campaigns.includes(map.campaignUid)) {
				campaigns.push(map.campaignUid);
			}

			if (!mapsMap[map.campaignUid]) {
				mapsMap[map.campaignUid] = [];
			}
			...
			>
				<div class="flex items-center justify-between mb-3">
					<h3 class="font-bold text-2xl text-gray-200">
						{campaign.split('-')[0].charAt(0).toUpperCase() + campaign.split('-')[0].slice(1)}
						{campaign.split('-')[1]}
					</h3>
					<div>
						<button
							disabled={isLoading}
							on:click={() => updateData(campaign)}
